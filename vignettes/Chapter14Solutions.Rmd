...
_The below code chunk takes up to a minute to run so it has not been evaluated here to shorten vignette compilation time_

```{r ex14.5, fig.width=9, fig.height=3.3, warning=FALSE, eval=FALSE}
par(mfrow=c(1,3),mar=c(3,3,1,1),mgp=c(1.75,0.75,0))
...
_The below code chunk takes almost a minute to run so it has not been evaluated here to shorten vignette compilation time_

```{r ex14.6assumptions, fig.width=9, fig.height=3.3, warning=FALSE, eval=FALSE}
par(mfrow=c(1,3),mar=c(3,3,1,1),mgp=c(1.75,0.75,0))
...
_The below code chunk takes a minute or so to run so it has not been evaluated here to shorten vignette compilation time_

```{r ex14.6refit, fig.width=9, fig.height=3.3, warning=FALSE, eval=FALSE}
par(mfrow=c(1,3),mar=c(3,3,1,1),mgp=c(1.75,0.75,0))
...
_This code chunk has not been run as it takes several minutes_
```{r ex14.8, eval=FALSE}
data(windFarms)
windMV1 = mvabund(windFarms$abund[,colSums(windFarms$abund>0)>1])
ft_wind1=manyglm(windMV1~Station+Year+Year:Zone, family="poisson", data=windFarms$X)
...
_This code chunk has not been run as it is reliant on the previous code chunk_
