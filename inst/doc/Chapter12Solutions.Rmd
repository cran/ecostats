...
```{r code12.1,fig.width=6,fig.height=7}
library(mvabund)
library(ecostats)
data(reveg)
...
```{r code12.2, fig.width=6, fig.height=6}
data("iris")
...
```{r code 12.4,fig.width=7,fig.height=7}
...
```
```{r code 12.4matrix,fig.width=7,fig.height=7}
...
```{r ex12.4plots, fig.width=8,fig.height=6}
...
of this solution:
```{r code12.6bi, fig.width=6, fig.height=6}
ordiplot(reveg_LVM, col=as.numeric(reveg$treatment), biplot=TRUE,
ind.spp=12)
```

```{r code12.6plot, fig.width=9}
...
```{r code12.7, fig.height=6, fig.width=6}
library(vegan)
...
```{r ex12.6ordination, fig.height=6, fig.width=6}
...
```{r ex12.6EucOrdination, fig.height=6, fig.width=6}
...
```{r ex12.6plot, fig.width=7,fig.height=8}
...
```{r ex12.6PA, fig.height=6, fig.width=6}
tikusPA = tikusAbund
tikusPA[tikusPA>1]=1
...
```{r ex12.6 assumptions, fig.width=9,fig.height=3.5}
par(mfrow=c(1,3))
