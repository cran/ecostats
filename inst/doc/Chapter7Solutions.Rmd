...
*Ian wanted to know: how does \emph{Myrtaceae} species richness vary from one area to the next, and what are the main environmental correlates of richness?... Plotting richness against spatial location, he found spatial clusters of high or low species richness (Figure~\ref{fig:richnessMap})... What sort of analysis method should Ian consider using?*
...
_The below code chunk takes several minutes to evaluate so it has not been run to shorten vignette compilation time_
```{r box75 spatial, eval = FALSE}
library(nlme)
...
_The below code chunk has not been run because of its reliance on the previous code chunk_
```{r box76, eval=FALSE} 
ft_richNugg
anova(ft_richAdd)
anova(ft_richNugg)
...
```{r box 78,fig.width=8,fig.height=9}
library(caper)
data(shorebird)
...
```{r box79, fig.width=7, fig.height=7}
library(GGally)
